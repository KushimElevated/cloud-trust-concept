import { TrustEngine, CONTROLS, PEOPLE, ROLES } from './engine.mjs';
...
function access(m) {
  return `${pageHeader('PILLAR 02 / ACCESS WHEN TRUSTED','Cloud Access','Privilege when justified. Gone when it isn’t.',button('Request access','request','primary','plus'))}
    <div class="access-summary"><div><span class="summary-icon">${icon('lock')}</span><span><strong>${m.active}</strong> active privileged session${m.active===1?'':'s'}</span></div><div class="access-summary-detail"><span>Standing grants <b>0</b></span><span>Maximum duration <b>60 min</b></span><span>Policy <b>v1.5</b></span></div></div>
...
function policies(embedded=false) {
  ...
  return `${embedded?'':pageHeader('SHARED CONTROL PLANE','Policies & exceptions','One set of trust conditions connects all four pillars.',badge('Policy v1.5','green'))}
...
function evidence(embedded=false) {
  ...
  return `${embedded?'':pageHeader('SHARED EVIDENCE','A continuous trust record','Every decision has an owner, a reason, and supporting context.',button('Export evidence','export','primary','download'))}
    <div class="evidence-summary"><span class="evidence-summary-icon">${icon('evidence')}</span><div><h2>${engine.events.length} recorded events</h2><p>Policy v1.5 · ${engine.environments.length} environments · ${engine.requests.length} access requests</p></div><span class="badge gray">Synthetic evidence</span></div>
...
document.addEventListener('click',e=>{
  ...
  try {
    ...
    else if(a==='advance'){engine.advance(Number(target.dataset.minutes));render();toast('Demo clock advanced by '+target.dataset.minutes+' minutes.');}
